...
#         self.train_images, self.train_labels = self.loader.get_test_set(True)
...
#     svm = SupportVectorMachine(loader, 3)
#     svm.start()
#     print('End training')
#     end = time.time()
...
for i in range(100):
    ...
    test_images[i] = image
for i in range(100):
    result = svm.predict(test_images[i])
    flag = (result >= 0)
    print(result)
    print('The prediction is 3:' + str(flag) + '. Truth is ' + str(test_labels[i]))
